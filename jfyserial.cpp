...
bool Serial::sendRequest( const Data& request )
{
	...
	if(verbose) {
		//fprintf(stderr,"wrote %u bytes to port\n",bytesWritten);
		cerr << "Tx:";
		int i;
		for(i=0;i<packetData.size();i++) {
			fprintf(stderr," %02X",packetData.data()[i]);
		}
		cerr << endl;
	}
...
Data Serial::readResponse()
{
	...
	try {
		...
		if (verbose) {
		  fprintf(stderr,"control0x%02X, function 0x%02X\n",header[4],header[5]);
		}
		...
		if ( dataSize > 0 ) {
			...
			if (verbose) {
				...
				for(i=0;i<dataSize;i++) {
					fprintf(stderr," %02X",responseData[i]);
				}
				cerr << endl;
			}

			//so a response with control code 0 and function 1 and data 15 means unsupported function code
			if (header[4]==0 && header[5]==1 && dataSize==1 && responseData[0]==NackResponseCode) {
				throw Exception("Inverter reports unrecognised function");
			}
